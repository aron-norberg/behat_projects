...
class MinkContext extends RawMinkContext implements TranslatableContext
{
     ...
     */
    public function clickCssLink($link)
    {
        $page = $this->getSession()->getPage();
        $link = $page->find('css', $link);
        echo $link = $link->getText();
        #echo $link = $link->getAttribute('href'); 
        $link = $this->fixStepArgument($link);
        $this->getSession()->getPage()->clickLink($link);
    }
     ...
     */
    public function printElementAttribute($element, $attribute)
    {
        $page = $this->getSession()->getPage();
    ...
    */
    public function elementshouldbetitlecased($element)
    {
        $page = $this->getSession()->getPage();
        $node = $page->find("css", $element);
        $node = $node->getText();
        ...
        #convert to Array
        $textArray = explode(" ",$node);

        if(ctype_upper($textArray[0][0]))
        {
            
        }else
        {
           echo "Element is not title Cased"; 
           assertFalse(true);
     ...
     */
    public function thePageTitleAndHShouldBeTheSame()
    {
        # This is the h1 tag
        $page = $this->getSession()->getPage();
        $node = $page->find('css', 'h1');
        $h1Node = $node->getText($node);

        # Get the title string
        $page = $this->getSession()->getPage();
        $node = $page->find('css', 'title');
        $node = $node->getText();
        ...
        # 
        $titleNode = implode(" ",$textArray);
        assertEquals($h1Node, $titleNode);
     ...
     */
    public function h1ElementShouldBeFormattedAsAnH1()
    {
        $page = $this->getSession()->getPage();
        $node = $page->find('css', 'h1');
        echo($node->getAttribute($attribute));
    ...
    * text snippet.
    * @Then check :textElement is in correct language
    */
    ...
    public function checkIsInCorrectLanguage($textElement)
    {
        ...
        # obtain the path from the url
        $urlArray = str_split($parsedUrl["path"]);
        # obtain the locale from the path and convert to an array
        $urlLocale = array_slice($urlArray,1,2);
        ...
        # langid should be run $langid --serve

        #get local ip where langid will be running
        $localIP = getHostByName(getHostName());

        $langidUrl = $localIP . ":9008/detect?q=";

        $langidUrl .= $encodedContent;

        $curl_options = array(
                    CURLOPT_URL => $langidUrl,
                    CURLOPT_HEADER => 0,
        ...
        $langIdResult = $jsonArray["responseData"]["language"];

        # Get the html lang attribute
        $htmlNode = $page->find('css', 'html');

        # slice needed characters and return to string
        $htmlNode = $htmlNode->getAttribute('lang');
        $htmlNodeArray = str_split($htmlNode);

        #slice needed characters and return to string
        $htmlLocale = array_slice($htmlNodeArray,0,2);
        $htmlLocale = implode($htmlLocale);

        assertEquals($langIdResult, $htmlLocale);
        assertEquals($htmlLocale, $urlLocale);
